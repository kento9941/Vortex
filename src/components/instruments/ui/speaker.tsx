interface SpeakerProps {
    play: boolean;
    onToggle: () => void;
}

const Speaker = ({ play, onToggle }: SpeakerProps) => {
    return (
        <button
            type="button"
            className="w-[1.2rem] flex justify-start cursor-pointer"
            onClick={onToggle}
            aria-label={play ? "Mute" : "Unmute"}
        >
            {play ? (
                // unmute icon
                <svg className="h-[0.8rem] aspect-[35/25]" viewBox="0 0 35 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M28.95 0.5C32.0745 3.62546 33.8298 7.86393 33.8298 12.2833C33.8298 16.7027 32.0745 20.9412 28.95 24.0667M23.0667 6.38333C24.6289 7.94606 25.5066 10.0653 25.5066 12.275C25.5066 14.4847 24.6289 16.6039 23.0667 18.1667M15.5 0.616668L7.16667 7.28333H0.5V17.2833H7.16667L15.5 23.95V0.616668Z" stroke="#C0C0C0" strokeWidth="0.15rem" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
            ) : (
                // mute icon
                <svg className="h-[0.8rem] aspect-[36/25]" viewBox="0 0 36 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M35.5 7.16667L25.5 17.1667M25.5 7.16667L35.5 17.1667M15.5 0.5L7.16667 7.16667H0.5V17.1667H7.16667L15.5 23.8333V0.5Z" stroke="#C0C0C0" strokeWidth="0.15rem" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
            )}
        </button>
    );
};

export default Speaker;